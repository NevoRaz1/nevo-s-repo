...
def calc_center_x(col, row, row_start):
    bubble_x = row_start + col * (consts.BUBBLE_RADIUS * 2 + consts.SPACE_BETWEEN_COLS) + consts.BUBBLE_RADIUS
...
def circleTouch(bubble1, bubble2):

    # Find the distance between the centers.
    dist = math.sqrt(1.0 * (bubble2["center_x"] - bubble1["center_x"]) * (bubble2["center_x"] - bubble1["center_x"]) + 1.0 * (bubble2["center_y"] - bubble1["center_y"]) * (bubble2["center_y"] - bubble1["center_y"]))

    # Check whether the circles have at least one common point.
    return (dist >= abs(bubble1['radius'] - bubble2['radius']) and dist <= bubble1['radius'] + bubble2['radius'])
